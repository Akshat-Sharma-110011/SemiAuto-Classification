...
"""
Enhanced Model Building Script with Complex Ensemble Support
Supports multi-stage stacking, advanced ensemble architectures, and custom ensemble topologies
"""
...
class ModelParameterValidator:
    """Validates and converts model parameters to correct types"""

    def __init__(self):
        # Define expected parameter types for each model class
        self.parameter_types = {
            'LogisticRegression': {
                'C': float, 'max_iter': int, 'solver': str, 'random_state': int, 'n_jobs': int,
                'penalty': str, 'tol': float, 'fit_intercept': bool, 'intercept_scaling': float,
                'class_weight': (str, dict, type(None)), 'dual': bool, 'multi_class': str,
                'warm_start': bool, 'l1_ratio': (float, type(None))
            },
            'RandomForestClassifier': {
                'n_estimators': int, 'max_depth': (int, type(None)), 'min_samples_split': int,
                'min_samples_leaf': int, 'max_features': (str, int, float, type(None)),
                'n_jobs': int, 'random_state': int, 'criterion': str,
                'max_leaf_nodes': (int, type(None)), 'min_impurity_decrease': float,
                'bootstrap': bool, 'oob_score': bool, 'warm_start': bool,
                'class_weight': (str, dict, type(None)), 'ccp_alpha': float,
                'max_samples': (int, float, type(None))
            },
            'XGBClassifier': {
                'n_estimators': int, 'learning_rate': float, 'max_depth': int,
                'subsample': float, 'colsample_bytree': float, 'objective': str,
                'n_jobs': int, 'eval_metric': str, 'random_state': int, 'verbosity': int,
                'reg_alpha': float, 'reg_lambda': float, 'gamma': float,
                'min_child_weight': int, 'colsample_bylevel': float,
                'colsample_bynode': float, 'scale_pos_weight': float,
                'base_score': float, 'booster': str, 'tree_method': str, 'num_class': int
            },
            'CatBoostClassifier': {
                'iterations': int, 'learning_rate': float, 'depth': int,
                'l2_leaf_reg': float, 'loss_function': str, 'verbose': (bool, int),
                'random_state': int, 'border_count': int, 'bagging_temperature': float,
                'random_strength': float, 'one_hot_max_size': int, 'rsm': float,
                'nan_mode': str, 'leaf_estimation_method': str, 'thread_count': int,
                'od_type': str, 'od_wait': int
            },
            'LGBMClassifier': {
                'n_estimators': int, 'learning_rate': float, 'max_depth': int,
                'num_leaves': int, 'subsample': float, 'colsample_bytree': float,
                'objective': str, 'n_jobs': int, 'random_state': int, 'verbose': int,
                'reg_alpha': float, 'reg_lambda': float, 'min_split_gain': float,
                'min_child_weight': float, 'min_child_samples': int,
                'subsample_freq': int, 'colsample_bylevel': float, 'max_bin': int,
                'num_class': int
            },
        }
    ...
    def validate_and_convert_parameters(self, model_class_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """Validate and convert parameters for a specific model class"""
...
class MultiStageStackingClassifier(BaseEstimator, ClassifierMixin):
    """
    Multi-stage stacking classifier supporting complex ensemble architectures
    like Jeong Yoon Lee's stacking model with multiple stages and diverse models
    """

    def __init__(self, stages_config: List[Dict], final_estimator=None, cv=5, n_jobs=None, passthrough=False):
        """
        Initialize multi-stage stacking classifier

        Parameters:
        -----------
        stages_config : List[Dict]
            Configuration for each stage containing estimators and their parameters
        final_estimator : object, optional
            Final estimator for the last stage
        cv : int, default=5
            Cross-validation strategy
        n_jobs : int, optional
            Number of jobs for parallel processing
        passthrough : bool, default=False
            Whether to include original features in final stage
        """
    ...
    def _create_estimator_from_config(self, estimator_config: Dict, available_models: Dict):
        """Create estimator instance from configuration"""
    ...
    def fit(self, X, y, available_models):
        """Fit the multi-stage stacking classifier"""
        ...
        for stage_idx, stage_config in enumerate(self.stages_config):
            ...
            # Create and fit estimators for this stage
            for estimator_config in stage_config['estimators']:
                ...
                # Generate cross-validated predictions for next stage
                if hasattr(estimator, 'predict_proba'):
                    cv_pred = cross_val_predict(estimator, X_current, y, cv=self.cv, method='predict_proba')
                    if cv_pred.shape[1] > 2:  # multiclass
                        stage_predictions.append(cv_pred)
                    else:  # binary
                        stage_predictions.append(cv_pred[:, 1:2])  # keep only positive class prob
                else:
            ...
            if stage_idx < len(self.stages_config) - 1:  # Not the last stage
        ...
        # Fit final estimator if provided
    ...
    def predict(self, X):
        """Make predictions using the multi-stage stacking classifier"""
        X_current = X.copy()
    ...
    def predict_proba(self, X):
        """Predict probabilities using the multi-stage stacking classifier"""
        if self.final_stage_ is not None and hasattr(self.final_stage_, 'predict_proba'):
            ...
            # Fallback to average probabilities from last stage
            X_current = X.copy()
...
class AdvancedEnsembleBuilder:
    """Builder for advanced ensemble models with complex architectures"""
    ...
    def _create_model_instance(self, model_config: Dict, y_train: pd.Series = None) -> Any:
        ...
        validated_params = self.parameter_validator.validate_and_convert_parameters(
            model_class_name, model_params
        )

        # Special handling for models that need multiclass objective adjustment
    ...
    def create_voting_ensemble(self, models_config: List[Dict], voting: str = 'soft',
                               y_train: pd.Series = None) -> VotingClassifier:
    ...
    def create_simple_stacking_ensemble(self, base_models_config: List[Dict],
                                        meta_model_config: Dict, cv: int = 5,
                                        y_train: pd.Series = None) -> StackingClassifier:
        """Create a traditional two-level stacking classifier"""
        estimators = []
    ...
    def create_multi_stage_stacking_ensemble(self, stages_config: List[Dict],
                                             final_estimator_config: Dict = None,
                                             cv: int = 5, passthrough: bool = False,
                                             y_train: pd.Series = None) -> MultiStageStackingClassifier:
        """Create a multi-stage stacking classifier"""
        final_estimator = None
        if final_estimator_config:
            final_estimator = self._create_model_instance(final_estimator_config, y_train)

        return MultiStageStackingClassifier(
    ...
    def create_bagging_ensemble(self, model_config: Dict, n_estimators: int = 10,
                                y_train: pd.Series = None) -> BaggingClassifier:
    ...
    def create_hybrid_ensemble(self, ensemble_config: Dict, y_train: pd.Series = None):
        """Create hybrid ensemble combining multiple ensemble techniques"""
        ensemble_type = ensemble_config.get('type', 'hybrid')

        if ensemble_type == 'voting_stacking':
            # Create voting ensembles as base models, then stack them
            ...
            for i, voting_config in enumerate(voting_configs):
                voting_ensemble = self.create_voting_ensemble(
                    voting_config['models'],
                    voting_config.get('voting', 'soft'),
                    y_train
                )
            ...
            return StackingClassifier(
                estimators=base_ensembles,
                final_estimator=meta_model,
                cv=stacking_config.get('cv', 5)
            )

        elif ensemble_type == 'bagging_stacking':
            # Create bagging ensembles as base models, then stack them
            bagging_configs = ensemble_config.get('bagging_ensembles', [])
            stacking_config = ensemble_config.get('stacking_config', {})

            base_ensembles = []
            for i, bagging_config in enumerate(bagging_configs):
                bagging_ensemble = self.create_bagging_ensemble(
                    bagging_config['base_model'],
                    bagging_config.get('n_estimators', 10),
                    y_train
                )
...
class EnhancedModelBuilder:
    """Enhanced Model Builder with complex ensemble support"""
    ...
    def _get_available_models_corrected(self) -> Dict[str, Dict[str, Any]]:
        """Get all available classification models with corrected default parameters"""
        models = {
            # Basic linear models
            "Logistic Regression": {
            ...
            "Passive Aggressive": {
                ...
                "params": {
                    "C": 1.0, "max_iter": 1000, "tol": 1e-3, "random_state": 42
                },
    ...
    def get_available_models(self) -> Dict[str, Dict]:
        """API-friendly method to get available models"""
        models_info = {}
    ...
    def build_ensemble_model(self, ensemble_config: Dict[str, Any]) -> Dict[str, Any]:
        """Build various types of ensemble models with improved error handling"""
        ...
        try:
            ...
            if ensemble_type == 'voting':
                ...
                model = self.ensemble_builder.create_voting_ensemble(
                    ensemble_config['models'],
                    ensemble_config.get('voting', 'soft'),
                    y_train
                )

            elif ensemble_type == 'stacking':
                logger.info("Creating stacking classifier ensemble")
                model = self.ensemble_builder.create_simple_stacking_ensemble(
                    ensemble_config['base_models'],
                    ensemble_config['meta_model'],
                    ensemble_config.get('cv', 5),
                    y_train
                )

            elif ensemble_type == 'multi_stage_stacking':
                logger.info("Creating multi-stage stacking classifier ensemble")
                model = self.ensemble_builder.create_multi_stage_stacking_ensemble(
                    ensemble_config['stages'],
                    ensemble_config.get('final_estimator'),
                    ensemble_config.get('cv', 5),
                    ensemble_config.get('passthrough', False),
                    y_train
                )

            elif ensemble_type == 'bagging':
                logger.info("Creating bagging classifier ensemble")
                model = self.ensemble_builder.create_bagging_ensemble(
                    ensemble_config['base_model'],
                    ensemble_config.get('n_estimators', 10),
                    y_train
                )
            ...
            if isinstance(model, MultiStageStackingClassifier):
    ...
    def build_single_model(self, model_name: str, custom_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if custom_params:
            validated_custom_params = self.parameter_validator.validate_and_convert_parameters(
                model_info['class'].__name__, custom_params
            )
    ...
    def _create_model_instance(self, model_name: str, model_info: Dict[str, Any], y_train: pd.Series) -> Any:
        """Create a model instance with proper configuration and parameter validation"""
        validated_params = self.parameter_validator.validate_and_convert_parameters(
            model_info['class'].__name__, model_info['params'].copy()
        )
    ...
    def select_best_model(self, parallel_results: Dict[str, Any], selection_metric: str = 'accuracy') -> Dict[str, Any]:
        """Select the best model from parallel training results"""
        ...
        if best_model_id:
            ...
            return {
                ...
                'model_path': model_path,
                'metrics': parallel_results['results'][best_model_id]['metrics'],
                'status': 'success'
            }
        else:
...
class ParallelModelTrainer:
    """Handles parallel training and evaluation of multiple models"""
    ...
    def train_models_parallel(self, models_dict: Dict[str, Any], max_workers: int = 4) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_model = {
                executor.submit(self.train_and_evaluate_model, model_id, model): model_id
...
def build_single_model_api_patched(model_name: str, custom_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """API endpoint for building single model with parameter validation"""
...
def build_parallel_models_api_patched(models_config: List[Dict[str, Any]], max_workers: int = 4) -> Dict[str, Any]:
    """API endpoint for building parallel models with parameter validation"""
    builder = EnhancedModelBuilder()
...
def build_ensemble_model_api(ensemble_config: Dict[str, Any]) -> Dict[str, Any]:
    """API endpoint for building ensemble models with improved error handling"""
    builder = EnhancedModelBuilder()
    ...
    try:
        ...
        if ensemble_type == 'voting':
            ...
            if 'n_estimators' not in ensemble_config:
                ensemble_config['n_estimators'] = 10  # default value
...
def select_best_model_api_patched(parallel_results: Dict[str, Any], selection_metric: str = 'accuracy') -> Dict[
    str, Any]:
    """API endpoint for selecting best model with validation"""
    builder = EnhancedModelBuilder()
    return builder.select_best_model(parallel_results, selection_metric)


if __name__ == "__main__":
    print("Enhanced Model Building Script with Complex Ensemble Support")
    print("Features:")
    print("- Multi-stage stacking ensembles")
    print("- Hybrid ensemble architectures")
    print("- Complex ensemble topologies")
    print("- Parameter validation and conversion")
    print("- Parallel model training")
    print("- Advanced ensemble builders")